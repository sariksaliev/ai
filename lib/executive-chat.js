...
function processChatMessage(data, message) {
  ...
  // Generate agent collaboration conversation
  const collaboration = simulateAgentCollaboration(data, question, involvedAgents);
...
function createInvestigationFromChat(data, question, collaboration) {
  const investigation = {
    ...
    question,
    status: 'draft',
    confidence: Math.round((0.78 + Math.random() * 0.15) * 100),
