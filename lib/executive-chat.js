...
const AGENTS = {
  ...
  knowledge: {
    ...
    letter: 'K',
    color: 'finance-orb',
    expertise: 'Company memory, strategic knowledge, verified sources',
    tools: ['Knowledge Base', 'Source Verifier', 'Memory Index', 'Freshness Monitor']
  }
};
...
async function processChatMessage(data, message) {
  ...
  const targetAgentId = message.agentId || null; // Support direct agent chat
  
  // Generate conversation ID
  const conversationId = message.conversationId || `conv_${Date.now()}`;
  
  // If targeting a specific agent, generate single-agent response via OpenRouter
  if (targetAgentId && AGENTS[targetAgentId]) {
    return await generateSingleAgentResponse(data, question, targetAgentId, conversationId, context);
  }
...
/**
 * Generate response from a single agent via OpenRouter API (with fallback to simulation)
 */
async function generateSingleAgentResponse(data, question, agentId, conversationId, context) {
  const agent = AGENTS[agentId];
  
  // Try real AI via OpenRouter first
  const { generateAgentResponse } = require('./ai-provider');
  let analysis, findings, recommendations;
  let aiEnabled = false;
  
  try {
    const aiResult = await generateAgentResponse(agentId, question, context);
    if (aiResult && aiResult.content) {
      analysis = aiResult.content;
      aiEnabled = true;
      // Try structured extraction from AI response
      findings = {
        confidence: 0.85,
        keyInsight: analysis.substring(0, 100),
        dataPoints: []
      };
      recommendations = analysis.length > 150 ? analysis : generateAgentRecommendation(agentId, question);
    } else {
      throw new Error('AI response empty');
    }
  } catch (e) {
    // Fallback to simulation
    analysis = generateAgentAnalysis(agent, question);
    findings = generateAgentFindings(agent, question);
    recommendations = generateAgentRecommendation(agentId, question);
  }
  
  // Write to blackboard
  const { writeToBlackboard } = require('./shared-blackboard');
  writeToBlackboard(agentId, { text: `Question: ${question}\nAnalysis: ${analysis}` }, { type: 'conversation', tags: ['chat', agentId], confidence: findings.confidence });
  ...
  return {
    ...
    response: {
      ...
      findings: {
        ...
        keyInsight: findings.keyInsight,
    ...
    suggestions: [
      ...
      `Покажи данные по этому вопросу`,
      `Что думают другие агенты?`
    ]
  };
}

function generateAgentRecommendation(agentId, question) {
  const recs = {
    sales: `Рекомендую: восстановить SLA ответа на лиды (17 лидов пропустили), провести deal review для 3 застрявших enterprise-сделок, ускорить сделку Acme Corp.`,
    marketing: `Рекомендую: скорректировать аудиторию Google Ads, перераспределить $8k в performance-каналы, запустить EU ретаргетинг.`,
    finance: `Рекомендую: мониторить cash runway, оптимизировать EU операции (на 8% менее эффективны), подготовить сценарии сокращения burn rate.`,
    customer: `Рекомендую: провести executive meeting с 3 аккаунтами под риском, запустить программу удержания, предложить expansion Acme Corp.`,
    operations: `Рекомендую: разблокировать security review, автоматизировать SLA мониторинг, оптимизировать кросс-функциональные процессы.`,
    ceo: `Рекомендую: утвердить комплексный план восстановления, назначить владельцев по каждому направлению, мониторить ежедневно через KPI Prediction.`,
    knowledge: `Рекомендую: обновить стратегические заметки по EU pipeline, зафиксировать выводы расследования в памяти компании, проверить свежесть 3 ключевых источников.`
  };
  return recs[agentId] || `Анализ выполнен на основе текущих данных. Рекомендуется дальнейшее исследование.`;
}
...
function generateAgentAnalysis(agent, question) {
  const templates = {
    ...
    knowledge: [
      'Проверяю память компании: 4,812 проверенных источников, свежесть знаний 96%.',
      'Нашёл релевантные заметки по EU pipeline и предыдущим расследованиям Q2.',
      'Сверяю стратегические цели с текущими инициативами в business memory.',
      'Индексирую выводы агентов: 3 связанных паттерна по спросу и SLA ответа.'
    ]
  };
...
function generateAgentFindings(agent, question) {
  const findings = {
    ...
    knowledge: {
      ...
      keyInsight: 'Релевантная память по EU pipeline и SLA найдена; 96% источников свежие',
      dataPoints: ['4,812 verified sources', '96% knowledge freshness', '3 linked investigation notes']
    }
  };
...
function createInvestigationFromChat(data, question, collaboration) {
  ...
  if (!Array.isArray(data.investigations)) data.investigations = [];
